...
// Multi-tenant helpers (unchanged)
...
/* NEW: Enforce 1 ATC per brand+session */
eventSchema.index(
  { brand_id: 1, session_id: 1, event_name: 1 },
...
const EventSchema = z.object({
  ...
  // NEW: accept session_id
  session_id: z.string().optional().nullable(),
  client_id: z.string().nullable(),
  visitor_id: z.string().nullable(),
  // make context fields tolerant
...
app.post('/collect', brandAuth, async (req, res) => {
  try {
    ...
    // NEW: always attach to a session
    let sessionId = e.session_id || null;

    // try reuse actor session within TTL
    if (!sessionId && actor) {
    ...
    // if still missing, create an anonymous session
    if (!sessionId) {
      sessionId = uuid();
    ...
    if (e.event_name === 'product_added_to_cart') {
      // NEW: only first ATC per session (per brand)
      await Event.updateOne(
        { brand_id: req.brand, session_id: sessionId, event_name: 'product_added_to_cart' },
        { $setOnInsert: baseDoc },
        { upsert: true }
      );
    } else {
      // default idempotency by event_id
      await Event.updateOne(
        { event_id: e.event_id },
        { $setOnInsert: baseDoc },
        { upsert: true }
...
(async () => {
  await mongoose.connect(MONGO_URI, { serverSelectionTimeoutMS: 10000, maxPoolSize: 10 });
  try {
