...
// ✅ NEW: prevent multiple PV/ATC per session
eventSchema.index(
  { session_id: 1, event_name: 1 },
  { unique: true, partialFilterExpression: { event_name: { $in: ["page_viewed", "product_added_to_cart"] } } }
);
...
app.post('/collect', brandAuth, async (req, res) => {
  try {
    ...
    // ✅ NEW: dedupe PV + ATC by session_id + event_name
    if (["page_viewed", "product_added_to_cart"].includes(e.event_name)) {
      await Event.updateOne(
        { session_id: sessionId, event_name: e.event_name },
        {
          $setOnInsert: {
            brand_id: req.brand,
            event_id: e.event_id,
      ...
      // everything else dedupes by event_id
      await Event.updateOne(
        { event_id: e.event_id },
        {
          $setOnInsert: {
